...
import type { BypassCategory, BypassOptions, BypassReport, BypassState } from '@/shared/types';
import type { Result, XrmReport } from '@/shared/types';

interface Props {
  /** Hands a generated bypass script to the Scripts tab so it can be saved and re-run. */
  onSaveScript: (name: string, code: string) => void;
}

const TOGGLES: { key: keyof BypassOptions; label: string; hint: string }[] = [
  {
...
const CATEGORY_LABELS: Record<BypassCategory, string> = {
...
function ReportView({ report }: { report: BypassReport }): ReactElement {
  const rows = (Object.keys(CATEGORY_LABELS) as BypassCategory[]).filter(
    (k) => report.counts[k] > 0
  );
  return (
...
export function BypassTool({ onSaveScript }: Props): ReactElement {
  const [options, setOptions] = useState<BypassOptions>(DEFAULT_BYPASS_OPTIONS);
  const [shouldWatch, setShouldWatch] = useState(false);
  const [state, setState] = useState<BypassState | null>(null);
  ...
  // Re-sync on mount: the content script owns the bypass, so it survives the
  ...
  const applyBypass = (): Promise<void> =>
    run<BypassReport>(
      () =>
        sendRuntimeMessage({
  ...
  const bypassDynamics = (): Promise<void> =>
    run<XrmReport>(() => sendRuntimeMessage({ type: MESSAGE_TYPES.BYPASS_XRM }), setXrm);
  ...
  return (
    <>
      <div className="row">
        ...
        >
          {isActive ? 'Bypass again' : 'Bypass page'}
        </button>
        <button type="button" disabled={isBusy || !isActive} onClick={() => void restore()}>
          Restore
        </button>
      </div>
      ...
      {state?.hasXrm === true && (
        <div className="row">
          <button type="button" disabled={isBusy} onClick={() => void bypassDynamics()}>
            Bypass Dynamics form
          </button>
        </div>
      )}
      ...
      <div className="row">
        <button
          type="button"
          onClick={() => onSaveScript('Bypass page', buildBypassScript(options))}
        >
          Save as a script
        </button>
      </div>
      <p className="hint">
        Saves these settings as a standalone script in the Scripts tab, so you can re-run the bypass
