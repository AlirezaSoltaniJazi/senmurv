...
| Form fields filled by **Recorder → Ad-hoc Insert**                           | Written to the active tab only when you click **Fill now**, using data generated on your device                                                                                                                                                                                                                                   | No               |
| Your interactions captured by **Recorder**                                   | Clicks, inputs and selects on the active tab are captured only while you are actively recording (**Record** until **Stop**), to build an editable step list. Nothing is stored until you click **Save**, and nothing leaves your device.                                                                                          | No               |
...
| Cookies read/edited in the **Cookies** tab                                   | Read and written on the current site only, using the `cookies` permission, including HttpOnly cookies a page script cannot see. Changes apply only when you save/add/delete a row. Nothing is stored by the extension beyond the cookie itself, and nothing is transmitted.                                                       | No               |
| Site storage read/edited in the **Storage** tab                              | The current site's localStorage and sessionStorage, read and written only on the tab you're viewing, only when you edit/add/delete an entry. Nothing is transmitted.                                                                                                                                                              | No               |
| Value profiles (**Cookies** / **Storage**)                                   | Saved cookie/localStorage/sessionStorage value presets you create, stored in `chrome.storage.local` (your browser profile). Applying one writes the value to that site's cookie/storage only when you click it.                                                                                                                   | No               |
...
| `cookies`                    | Lets the **Cookies** tab read and write the current site's cookies, including HttpOnly ones a page script cannot see.                                                                                          |
