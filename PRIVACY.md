...
| Page changes made by **Tools → Unlock**                | Attributes on the active tab are modified only when you click **Unlock**, and the original values are kept in memory so **Restore** can put them back. Nothing is stored or transmitted; the counts shown are numbers only, never page content. Revealing password fields is off by default. | No               |
